# Changelog

All notable changes to this project will be documented in this file.

## 1.0.0 - 2026-09-03

- Add five advisory, evidence-led sales skills.
- Add standalone and Marketing Skills coexistence routing.
- Add untrusted-artifact, authority, consent, fairness, and anti-manipulation boundaries.
- Add behavioral fixtures, dependency-free structural validation, and clean-room overlap checking.
- Bound overlap-checker memory and report retention while preserving exact, collision-safe counts.
- Reject malformed source-audit and coexistence fixture types without crashing validation.
- Fail closed on excessive, partial, or symlink-contaminated repository traversal, unreadable or invalid-UTF-8 input, decoder depth, invalid decoded link paths, mutable workflow-action literals, inexact provenance maps, and impractical overlap windows.
- Add public contribution, security-reporting, ownership, funding, dependency-update, and CodeQL configuration.
- Require immutable commit pins for third-party GitHub Actions.
